...
export const VSLPage: React.FC<VSLPageProps> = ({ navigate, openExitModal }) => {
  ...
  const whatsappInquiryUrl = `https://wa.me/${COMPANY_DETAILS.whatsappRaw}?text=${encodeURIComponent(
    `Hello Milnapath Mentor, I am currently watching Masterclass Module ${activeModule.id}: "${activeModule.title}" and I would like to ask some questions before joining.`
  )}`;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-950 text-white rounded-2xl p-4 sm:p-5 border border-emerald-700/60 shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 text-left">
          ...
          <div>
            <h2 className="font-bold text-sm sm:text-base font-serif text-white">
              Milnapath Masterclass & Mentorship Video Vault
            </h2>
            <p className="text-xs text-emerald-200">
              Complete private orientation for prospective team leaders & distributors.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-emerald-950 font-bold text-xs rounded-xl shadow-md transition-all flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="relative aspect-video bg-stone-950 rounded-3xl overflow-hidden shadow-2xl border-4 border-emerald-800/60 flex flex-col justify-between group">
            {/* Visual Screen Poster / Mock Player */}
            <div className="absolute inset-0 bg-gradient-to-t from-black via-emerald-950/80 to-stone-950 flex items-center justify-center p-6 text-center">
              {/* Background decorative diagram */}
              <div className="absolute inset-0 opacity-15 pointer-events-none flex items-center justify-center">
                <div className="w-[500px] h-[500px] border-8 border-emerald-500 rounded-full animate-spin-slow"></div>
              </div>

              <div className="relative z-10 space-y-4 max-w-xl">
                <div className="inline-flex items-center gap-2 bg-emerald-900/80 border border-emerald-500/40 text-emerald-300 px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-sm">
                  <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
                  <span>MODULE {activeModule.id} OF 10: {activeModule.category.toUpperCase()}</span>
                </div>
                ...
                <p className="text-xs sm:text-sm text-emerald-100/80 max-w-md mx-auto line-clamp-2">
            ...
            <div className="relative z-20 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4 sm:p-5 space-y-2 mt-auto">
              ...
              <div className="flex items-center justify-between text-white text-xs pt-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="hidden sm:inline-block text-[11px] bg-emerald-900/80 text-emerald-300 px-2 py-0.5 rounded font-medium border border-emerald-700/50">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-lg border border-stone-200/80 space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-stone-100">
              <div>
                ...
                <h3 className="text-xl font-bold font-serif text-emerald-950 mt-1">
                  {activeModule.title}
                </h3>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1 bg-stone-100 text-stone-700 text-xs px-3 py-1 rounded-full font-medium">
                  <Clock className="w-3.5 h-3.5 text-stone-500" />
                  {activeModule.duration}
                </span>
                <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-800 text-xs px-3 py-1 rounded-full font-semibold border border-emerald-100">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center gap-4">
              <button
                onClick={() => navigate('/apply')}
                className="w-full sm:w-auto flex-1 py-4 px-6 bg-gradient-to-r from-emerald-800 via-emerald-700 to-emerald-900 hover:from-emerald-700 hover:to-emerald-800 text-white font-bold rounded-2xl text-sm shadow-xl shadow-emerald-900/20 flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
              ...
              <a
                ...
                className="w-full sm:w-auto py-4 px-6 bg-emerald-50 hover:bg-emerald-100 text-emerald-950 font-bold rounded-2xl text-sm border border-emerald-300 flex items-center justify-center gap-2 transition-colors"
              >
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-stone-200/80 p-5 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-stone-100">
              <div>
                ...
                <p className="text-[11px] text-stone-500">10 Comprehensive Training Videos</p>
              </div>
            ...
            <div className="pt-2 border-t border-stone-100">
              <button
                ...
                className="w-full py-3 bg-amber-500 hover:bg-amber-400 text-emerald-950 font-extrabold rounded-xl text-xs shadow-md transition-all flex items-center justify-center gap-1.5"
              >
